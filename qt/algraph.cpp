    ...
    : QObject{parent}
{
    ...
    addLineFromFile("Flight.txt");
    addLineFromFile("Train.txt");
}
...
void ALGraph::addCityFromFile(const char FILENAME[MAXFILESIZE]) {
    qDebug() << "开始从" << QString::fromStdString(FILENAME) << "中读入并添加城市！" ;
...
void ALGraph::addLineFromFile(const char FILENAME[MAXFILESIZE]) {
    ...
    if (!file) {
        qDebug() << "打开文件" << QString::fromStdString(FILENAME) << "失败！" ;
        return;
    }
    ...
    qDebug() << "从文件" << QString::fromStdString(FILENAME) << "中导入了 "
...
void ALGraph::delLine(const string& sc, const string& ec, const string& amt) {
    ...
    qDebug() << "正在删除从" << QString::fromStdString(sc) << "到" << QString::fromStdString(ec)
             << "，班次号为" << QString::fromStdString(amt) << "的线路！" ;
